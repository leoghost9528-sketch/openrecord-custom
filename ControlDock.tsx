...
interface ControlDockProps {
  ...
  onChangeCamSize: (size: number) => void;
  onStartRecord: () => void;
  onStopRecord: () => void;
          ...
          <>
            ...
            <div className="flex items-center gap-1 pr-3 border-r border-gray-300/50">
                ...
                <div className="flex gap-1 ml-1 pl-1 border-l border-gray-300/30">
                    ...
                    {!isFullCam && (
                        <div className="relative group">
                            <button className="p-2.5 rounded-xl hover:bg-black/5 text-gray-700 transition-colors flex items-center gap-1" title="Camera Size">
                                <span className="text-xs font-semibold">⊕</span>
                            </button>
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 pb-3 hidden group-hover:block z-50">
                                <div className="flex flex-col bg-white/90 backdrop-blur-md rounded-xl shadow-xl border border-white/20 p-1 min-w-[100px]">
                                    <button onClick={() => onChangeCamSize(250)} className={`px-3 py-2 text-xs font-medium hover:bg-blue-50 text-left rounded-lg ${camSize === 250 ? 'bg-blue-50' : ''}`}>Small</button>
                                    <button onClick={() => onChangeCamSize(350)} className={`px-3 py-2 text-xs font-medium hover:bg-blue-50 text-left rounded-lg ${camSize === 350 ? 'bg-blue-50' : ''}`}>Medium</button>
                                    <button onClick={() => onChangeCamSize(450)} className={`px-3 py-2 text-xs font-medium hover:bg-blue-50 text-left rounded-lg ${camSize === 450 ? 'bg-blue-50' : ''}`}>Large</button>
                                    <button onClick={() => onChangeCamSize(550)} className={`px-3 py-2 text-xs font-medium hover:bg-blue-50 text-left rounded-lg ${camSize === 550 ? 'bg-blue-50' : ''}`}>X-Large</button>
                                </div>
                            </div>
                        </div>
                    )}
